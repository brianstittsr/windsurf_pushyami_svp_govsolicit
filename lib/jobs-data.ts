...
// Sample job listings - in production, these would come from a database
...
export function getActiveJobs(): JobListing[] {
  return sampleJobListings.filter(job => job.status === "active");
}

// Helper function to get job by ID
export function getJobById(id: string): JobListing | undefined {
  return sampleJobListings.find(job => job.id === id);
}

// Helper function to filter jobs
export function filterJobs(
  jobs: JobListing[],
  filters: { department?: string; location?: string; type?: string; search?: string }
): JobListing[] {
  return jobs.filter(job => {
    ...
    if (filters.search) {
      ...
      const matchesDepartment = job.department.toLowerCase().includes(searchLower);
      if (!matchesTitle && !matchesDescription && !matchesDepartment) return false;
    }
    return true;
  });
}
